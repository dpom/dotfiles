...
- **THEN** the Ollama package SHALL be built with ROCm dependencies (`rocmPackages.clr`, `rocmPackages.hipblas`, `rocmPackages.rocblas`)
- **THEN** `hardware.graphics.extraPackages` SHALL include ROCm ICD and runtime packages
- **THEN** the ollama service user SHALL be in the `video` and `render` groups
