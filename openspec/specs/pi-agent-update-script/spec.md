# pi-agent-update-script

## Purpose

Provide a `bin/update-pi` script that resolves the latest releases of both pinned pi components (`earendil-works/pi` and `svkozak/pi-acp`) from GitHub and updates the pi module pins in the literate Org source, regenerating the module without deploying or committing.
